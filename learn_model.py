...
n = ['text']
data_positive = pd.read_csv('combined_output_positive.csv', sep=';',  names=n, usecols=['text'])
data_negative = pd.read_csv('combined_output_negative.csv', sep=';',  names=n, usecols=['text'])
...
epochs = 1 #оптимальное количество эпох для того, чтобы модель достаточно обучилась, но не переобучилась
counter = 0
batch_num = 100
clip = 5
if (train_gpu):
